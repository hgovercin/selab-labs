...
- [ ] Er is een volledig werkende virtuele machine volgens de eindsituatie.
  ...
  - [ ] De Docker services zijn correct geconfigureerd:
    - [ ] Vaultwarden is bereikbaar op <https://192.168.56.20:4123> (`troubleshoot@selabs.hogent.be shootthetrouble`).
...
- Webserver (Apache2)
  ...
  - Het is mogelijk om via de Ubuntu-gebruiker bestanden naar de webserver (in map `/var/www`) te uploaden via WinSCP (of een gelijkaardige tool) vanuit de hostomgeving via 192.168.56.20, poort 22 (via SFTP). ⚠️ Overschrijf echter niet het door ons aangeleverde `index.html` bestand.
